"""
透過藝術的孩童教育 · 情境01 — 繪畫發展的三階段
產生每句一個 mp3 (結尾含靜音) + lines.json
用法: python generate_audio.py
"""

import asyncio
import json
import os
import edge_tts

ZH_LINES = [
    "孩童的發展階段大致可分為三期、",
    "第一期是如有似無目的的錯晝期、",
    "在0-3歲又稱塗鴉期、",
    "有些孩童會延伸到5歲。",
    "第二階段是畫線期、",
    "在4歲左右的孩童最喜歡晝人、",
    "但是人的身體四肢很少被完整畫出來。",
    "第三階段是圖式期、",
    "在6歲左右的孩童依然興趣畫人物、",
    "但這個時候已經是經過思考後比較複雜的人像、",
    "每個孩童在每一階段停留的時間不一樣、",
    "這與其身心發展有相關性。"
]

EN_LINES = [
    "Children's developmental stages can broadly be divided into three phases.",
    "The first is the purposeless, seemingly random scribbling phase,",
    "occurring from zero to three years old, known as the scribbling stage,",
    "and some children extend it up to age five.",
    "The second phase is the line-drawing stage,",
    "around age four, when children most love to draw people,",
    "but the body's limbs are rarely drawn completely.",
    "The third phase is the schematic stage,",
    "around age six, when children still love to draw figures,",
    "but their figures are now more thoughtful and complex.",
    "Every child spends a different amount of time in each stage,",
    "which correlates with their physical and mental development."
]

ZH_VOICE = "zh-TW-HsiaoChenNeural"
EN_VOICE = "en-US-AriaNeural"
ZH_RATE = "-10%"
EN_RATE = "-5%"

TARGET_TAIL_SILENCE_MS = 150
SILENCE_THRESHOLD_DB = -45


async def synth_one(text, voice, rate, out_file):
    tts = edge_tts.Communicate(text, voice, rate=rate)
    await tts.save(out_file)


async def generate_lang(lines, voice, rate, prefix, label):
    from pydub import AudioSegment
    from pydub.silence import detect_leading_silence

    print(f"\n  [{label}] 產生 {len(lines)} 個句子檔案...")

    for i, line in enumerate(lines):
        idx = f"{i+1:02d}"
        tmp_file = f".tmp_{prefix}_{idx}.mp3"
        final_file = f"line_{idx}_{prefix}.mp3"

        await synth_one(line.strip(), voice, rate, tmp_file)

        try:
            audio = AudioSegment.from_mp3(tmp_file)
            original_len = len(audio)

            leading_silence = detect_leading_silence(audio, silence_threshold=SILENCE_THRESHOLD_DB)
            leading_silence = max(0, leading_silence - 50)
            if leading_silence > 0:
                audio = audio[leading_silence:]

            reversed_audio = audio.reverse()
            trailing_silence = detect_leading_silence(reversed_audio, silence_threshold=SILENCE_THRESHOLD_DB)
            if trailing_silence > 0:
                audio = audio[:-trailing_silence]

            if i < len(lines) - 1:
                audio = audio + AudioSegment.silent(
                    duration=TARGET_TAIL_SILENCE_MS,
                    frame_rate=audio.frame_rate
                )

            audio.export(final_file, format="mp3", bitrate="48k")
            print(f"    [{idx}] {line[:15]}... ({original_len}ms -> {len(audio)}ms)")
        except Exception as e:
            print(f"    WARN pydub 失敗,使用原檔: {e}")
            os.replace(tmp_file, final_file)
        else:
            try:
                os.remove(tmp_file)
            except OSError:
                pass

    print(f"  [{label}] OK")


async def main():
    print("=" * 50)
    print("情境01 (繪畫發展的三階段) 開始產生...")
    print("=" * 50)
    await generate_lang(ZH_LINES, ZH_VOICE, ZH_RATE, "zh", "中文")
    await generate_lang(EN_LINES, EN_VOICE, EN_RATE, "en", "英文")

    lines_data = []
    for i in range(len(ZH_LINES)):
        lines_data.append({
            "index": i,
            "zh": ZH_LINES[i],
            "en": EN_LINES[i] if i < len(EN_LINES) else ""
        })
    with open("lines.json", "w", encoding="utf-8") as f:
        json.dump(lines_data, f, ensure_ascii=False, indent=2)

    print("\n全部完成!")


if __name__ == "__main__":
    asyncio.run(main())
